...
def _settings():
    return SimpleNamespace(agent_model="m", resolved_light_model="m", light_reasoning=False,
...
def test_summarize_tool_call_builds_a_result_prompt_and_returns_stripped_content():
    ...
    with patch("novelizer.tui.tool_summarizer.build_light_model", return_value=fake_model) as build:
...
def test_summarize_tool_call_uses_the_error_line_when_the_call_failed():
    ...
    with patch("novelizer.tui.tool_summarizer.build_light_model", return_value=fake_model):
        result = asyncio.run(summarize_tool_call(
