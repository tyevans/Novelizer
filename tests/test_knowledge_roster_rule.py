"""The rule that every production caller of the knowledge-intents committer
supplies a character roster.

commit_knowledge_intents validates the secret id an agent cites, and -- when
given a `character_ids` roster -- that the cited character exists. The roster
is optional on purpose: plenty of tests commit intents with no cast to check
...
COMMITTER_NAMES = {"commit_knowledge_intents", "_commit_knowledge_intents"}
ROSTER_KWARG = "character_ids"


def _callee_name(node: ast.Call) -> str:
    """The bare function name of a call, however it is reached --
    `commit_knowledge_intents(...)`, `intent_helpers.commit_knowledge_intents(...)`
    and `self._commit_knowledge_intents(...)` all reduce to the same name."""
    func = node.func
...
def find_calls_missing_roster(source: str) -> list[int]:
    """Line numbers of calls to the knowledge committer that omit the roster.
...
def test_every_production_call_site_supplies_a_character_roster():
    ...
    assert offenders == [], (
        "these calls commit knowledge intents without a character roster, so a "
...
def test_at_least_one_production_call_site_is_actually_detected():
    ...
    }, f"the set of modules committing knowledge intents changed: {sorted(seen)}"
...
def test_the_detector_catches_a_call_site_that_forgets_the_roster():
    ...
    source = (
        "async def work(self):\n"
        "    await self._commit_knowledge_intents(intents, active_secret_ids)\n"
    )
...
def test_the_detector_accepts_a_forwarded_roster():
    forwarded = (
        "async def _commit_knowledge_intents(self, intents, ids, character_ids=None):\n"
        "    await intent_helpers.commit_knowledge_intents(\n"
        "        self._committer, self.name, intents, ids, character_ids=character_ids,\n"
        "    )\n"
    )
    assert find_calls_missing_roster(forwarded) == []
    splatted = "commit_knowledge_intents(c, name, intents, ids, **overrides)\n"
