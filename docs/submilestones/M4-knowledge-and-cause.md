...
3. **A leak is a precise, deterministic, testable condition: a committed
   ...
   *Implementation note (M4.2):* Leak evaluation runs against the current aggregate
   knowledge matrix at each Continuity Checker cycle, which always includes the
   referencing chapter's own commits (satisfying the same-chapter learn+use rule). When a
   `secret.learned` event lands in a strictly later chapter, it stops the leak from being
   re-flagged in subsequent cycles — per-chapter historical snapshots were deliberately
   not built (YAGNI, documented in `novelizer/brain/leaks.py`).
...
4. **Causal graph: edges are `(cause_chapter_id, effect_chapter_id, note)` triples declared
   ...
   needed are adjacency-list fold, ordering-violation check, and cycle detection (SCC-based
   via Tarjan's algorithm over a dict-of-lists, ~70 lines), and adding a graph library for that would be scope
...
### M4 closeout note (2026-07-18): part (b) live observation deferred by director decision

Part (a) — the CI mechanical chain — is fully proven (seeded leak →
`LeakDetector` flags → real `ContinuityChecker.run_once()` with a stubbed LLM
files a `LEAK_SOURCE_TAG` retcon request → open queue → Who-Knows-What still
shows the character as not-knowing). Part (b) — the live end-to-end pass —
was attempted extensively (20+ live runs against gemma-4-26b-qat) and never
completed, for a reason worth recording: **the model was consistently
semantically right at every turn.** The Author's injected `known_secrets_note`
reliably *prevented* Author-side leaks (raw reasoning showed deliberate
correct attribution to the knower every time — the guardrail demonstrably
works live); a public-announcement fixture was correctly annotated `reveal`,
not `uses`, by the live Editor. Every live link was individually observed
(real Author emits valid `knowledge_intents`; real Editor annotates prose
into knowledge intents once given the citation-aid id list added during this
closeout), but a full leak→catch pass awaits either a sloppier sample or the
current usage-not-revelation fixture in `tests/agents/test_leak_live_llm.py`,
which documents staged failure semantics and remains runnable with
`uv run pytest -m live_llm tests/agents/test_leak_live_llm.py`. Deferred to a
future reliability pass alongside the other live-observation follow-ups.
