...
| M9 | **Arcs** | Character arc aggregate (ghost/lie/want/need, five types), `ArcIntent` for the CharacterKeeper, `arc_alignment.py`, Arcs tab with pivots pinned to beats | A declared fall arc that resolves `truth_embraced` raises a contradiction alarm for the Director to adjudicate; a stagnant arc surfaces and the Plotter routes its character into the next brief | ✅ complete (pending review; relationship arcs + per-advance pivot history deferred; checker promise-mining + search kinds → M10; orphaned-pivot re-pin finding → M10) |
