...
| M11 | **Landing** | Endgame steering: completion criteria over the blueprint (beats fulfilled ∧ promises paid/released ∧ arcs resolved), `blueprint.retargeted` flow when the book runs long/short, finale-window convergence notes, `book.completed`, framing a story via the story picker (blueprint adoption at story creation), Outline board as the home Brain tab | A seeded story runs to a **finished** novel and the system declares it done; a stranger reads a book with a beginning, a middle, and an *ending* | ✅ complete (pending review; the day-long live acceptance run remains with the Director) |
